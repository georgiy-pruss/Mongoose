...
static char s_msg_file[PATH_MAX];     // Set by main()
...
#endif /* !CONFIG_FILE */
#define MESSAGE_FILENAME "mongoose.log"  /* in TMP if possible */
...
static void
vnotify( const char* fmt, va_list ap, int must_exit )
{
  FILE* f;
  char date[64];
  time_t now;
  f = fopen( s_msg_file, "at" );
  if( f )
  {
    now = time( NULL );
...
show_usage_and_exit( void )
{
  ...
  n += sprintf( out+n, "  mongoose -A <htpasswd_file> <realm> <user> <passwd>\n" );
#endif
  n += sprintf( out+n, "  mongoose [config_file]\n" );
  n += sprintf( out+n, "  mongoose [-option value ...]\n\n" );
  n += sprintf( out+n, "OPTIONS:\n" );
  names = mg_get_valid_option_names();
  for( i = 0; names[i] != NULL; i += 2 )
    n += sprintf( out+n, "  -%s %s\n", names[i], names[i+1] == NULL ?
        "<empty>" : names[i+1] );
  MessageBox( NULL, out, "Mongoose Web Server", MB_OK );
  exit( EXIT_FAILURE );
}
...
process_command_line_arguments( char* argv[], char** options )
{
  ...
  if( fp != NULL )
  {
    notify( "Loading config file %s", s_config_file );
...
static void*
serving_thread_func( void* param )
{
  struct mg_server* srv = (struct mg_server*)param;
  while( exit_flag == 0 && mg_shutdown == 0 )
    mg_poll_server( srv, 1000 );
  return NULL;
}

static int
path_exists( const char* path, int is_dir )
{
  file_stat_t st;
  return path == NULL ||
    stat( path, &st ) == 0 && (S_ISDIR( st.st_mode ) ? 1 : 0) == is_dir;
}

static void
verify_existence( char** options, const char* name, int is_dir )
{
...
static void
set_absolute_path( char* options[], const char* option_name )
{
...
#if !defined(MONGOOSE_NO_AUTH) && !defined(MONGOOSE_NO_FILESYSTEM)
int
modify_passwords_file( const char* fname, const char* domain,
...
static struct mg_server*
start_mongoose( int argc, char* argv[] )
{
  ...
  if( argc == 2 &&
      (!_stricmp( argv[1], "-h" ) || !strcmp( argv[1], "--help" ) ||
       !_stricmp( argv[1], "/h" ) || !strcmp( argv[1], "/?" )) )
    show_usage_and_exit();
...
static void
set_options( struct mg_server* server, char* argv[] )
{
  ...
  // Setup signal handler: quit on Ctrl-C (useless if windows WinMain app.)
...
int APIENTRY
WinMain( HINSTANCE hInst, HINSTANCE hPrevInst, LPTSTR lpCmdLine, int nCmdShow )
// int main(int argc, char *argv[])
{
  ...
  for( argc=1, done=0, b=p=cmdline; !done && argc<MAXARGS-1; ++p )
  {
    if( *p!=' ' && *p!='\0' ) continue;
    if( *p=='\0' ) done = 1;
    if( p<=b ) break;
  ...
  if( !p ) p = "C:";
  strcpy( s_msg_file, p );
  if( s_msg_file[strlen( s_msg_file )-1]!='\\' ) strcat( s_msg_file, "\\" );
  strcat( s_msg_file, MESSAGE_FILENAME );

  server = start_mongoose( argc, argv );
  snprintf( server_name, sizeof(server_name),
      "Mongoose web server v.%s", MONGOOSE_VERSION );
  notify( "Starting %s, Root %s, Port %s\n", server_name,
      mg_get_option( server, "document_root" ),
      mg_get_option( server, "listening_port" ) );
  serving_thread_func( server );
  if( mg_shutdown != 0 )
    notify( "Exiting on shutdown request" );
  else
    notify( "Exiting on signal %d", exit_flag );
