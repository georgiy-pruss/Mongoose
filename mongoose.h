...
const char* mg_set_option( struct mg_server*, const char* opt,
    const char* val );
...
size_t mg_url_encode( const char* src, size_t s_len, char* dst,
    size_t dst_len );
...
extern int mg_shutdown;  // shutdown flag, can be tested outside
extern const char mg_features[]; // features signature
