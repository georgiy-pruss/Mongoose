...
// All rights reserved
// Copyright (c) 2015 Georgiy Pruss - some modifications
//
...
#pragma warning (disable : 4996)  // deprecated getcwd and chdir
#endif
...
void
iobuf_resize( struct iobuf* io, size_t new_size )
{
  char* p;
...
void
iobuf_init( struct iobuf* iobuf, size_t initial_size )
{
...
size_t
iobuf_append( struct iobuf* io, const void* buf, size_t len )
{
  char* p = NULL;
...
void
iobuf_remove( struct iobuf* io, size_t n )
{
  if( n > 0 && n <= io->len )
  {
...
static size_t
ns_out( struct ns_connection* nc, const void* buf, size_t len )
{
  if( nc->flags & NSF_UDP )
  {
...
void*
ns_start_thread( void* (*f)( void* ), void* p )
{
#ifdef _WIN32
...
static void
ns_add_conn( struct ns_mgr* mgr, struct ns_connection* c )
{
...
static void
ns_remove_conn( struct ns_connection* conn )
{
...
int
ns_avprintf( char** buf, size_t size, const char* fmt, va_list ap )
{
  va_list ap_copy;
  int len;
...
int
ns_vprintf( struct ns_connection* nc, const char* fmt, va_list ap )
{
  char mem[NS_VPRINTF_BUFFER_SIZE];
  char* buf = mem;
  int len;
...
int
ns_printf( struct ns_connection* conn, const char* fmt, ... )
{
  int len;
  va_list ap;
  va_start( ap, fmt );
...
static void
hexdump( struct ns_connection* nc, const char* path,
...
static void
ns_call( struct ns_connection* nc, int ev, void* p )
{
...
static void
ns_destroy_conn( struct ns_connection* conn )
{
...
static void
ns_close_conn( struct ns_connection* conn )
{
...
void
ns_set_close_on_exec( sock_t sock )
{
#ifdef _WIN32
...
static void
ns_set_non_blocking_mode( sock_t sock )
{
#ifdef _WIN32
  unsigned long on = 1;
...
int
ns_socketpair2( sock_t sp[2], int sock_type )
{
...
static int
ns_resolve2( const char* host, struct in_addr* ina )
{
...
int
ns_resolve( const char* host, char* buf, size_t n )
{
  struct in_addr ad;
  return ns_resolve2( host, &ad ) ? snprintf( buf, n, "%s",
    inet_ntoa( ad ) ) : 0;
}

// Address format: [PROTO://][IP_ADDRESS:]PORT[:CERT][:CA_CERT]
static int
ns_parse_address( const char* str, union socket_address* sa,
...
static sock_t
ns_open_listening_socket( union socket_address* sa, int proto )
{
...
static int
ns_use_ca_cert( SSL_CTX* ctx, const char* cert )
{
  if( ctx == NULL )
    return -1;
...
static int
ns_use_cert( SSL_CTX* ctx, const char* pem_file )
{
  if( ctx == NULL )
    return -1;
  if( pem_file == NULL || pem_file[0] == '\0' )
    return 0;
  if( SSL_CTX_use_certificate_file( ctx, pem_file, 1 ) == 0 ||
...
struct ns_connection*
ns_bind( struct ns_mgr* srv, const char* str,
    ns_callback_t callback, void* user_data )
{
  ...
  if( (sock = ns_open_listening_socket( &sa, proto )) == INVALID_SOCKET )
    return NULL;
  if( (nc = ns_add_sock( srv, sock, callback, NULL )) == NULL )
  {
    closesocket( sock );
  }
  else
...
static struct ns_connection*
accept_conn( struct ns_connection* ls )
{
...
void
ns_sock_to_str( sock_t sock, char* buf, size_t len, int flags )
{
  union socket_address sa;
...
int
ns_hexdump( const void* buf, int len, char* dst, int dst_len )
{
...
#ifdef NS_ENABLE_SSL
static int
ns_ssl_err( struct ns_connection* conn, int res )
{
...
static void
ns_read_from_socket( struct ns_connection* conn )
{
...
static void
ns_write_to_socket( struct ns_connection* conn )
{
...
int
ns_send( struct ns_connection* conn, const void* buf, size_t len )
{
  return (int)ns_out( conn, buf, len );
}

static void
ns_handle_udp( struct ns_connection* ls )
{
...
static void
ns_add_to_set( sock_t sock, fd_set* set, sock_t* max_fd )
{
...
time_t
ns_mgr_poll( struct ns_mgr* mgr, int milli )
{
...
struct ns_connection*
ns_connect( struct ns_mgr* mgr, const char* address,
    ns_callback_t callback, void* user_data )
{
  sock_t sock = INVALID_SOCKET;
  struct ns_connection* nc = NULL;
...
struct ns_connection*
ns_add_sock( struct ns_mgr* s, sock_t sock,
    ns_callback_t callback, void* user_data )
{
...
struct ns_connection*
ns_next( struct ns_mgr* s, struct ns_connection* conn )
{
  return conn == NULL ? s->active_connections : conn->next;
}

void
ns_broadcast( struct ns_mgr* mgr, ns_callback_t cb, void* data, size_t len )
{
  struct ctl_msg ctl_msg;
...
void
ns_mgr_init( struct ns_mgr* s, void* user_data )
{
  memset( s, 0, sizeof(*s) );
...
} static_builtin_mime_types[] =
{
  ...
  { ".7z", 4, "application/x-7zip-compressed" },
...
mmap( void* addr, int64_t len, int prot, int flags, int fd, int offset )
{
  ...
  if( lseek( fd, offset, SEEK_SET ) == -1 ) return NULL;
  p = malloc( len );
  /* Read in a file */
  if( !p || read( fd, p, len ) == -1 ) { free( p ); p = NULL; }
...
static int
mg_stat( const char* path, file_stat_t* st )
{
...
static int
get_request_len( const char* s, size_t buf_len )
{
...
static void
parse_http_headers( char** buf, struct mg_connection* ri )
{
  size_t i;
...
static int
call_user( struct connection* conn, enum mg_event ev )
{
...
static void
send_http_error( struct connection* conn, int code,
    const char* fmt, ... )
{
  ...
  // G-: let's return just "[]" instead of full 404 message
  if( code == 404 )
...
static void
write_chunk( struct connection* conn, const char* buf, int len )
{
  char chunk_size[50];
...
static void
ns_forward( struct ns_connection* from, struct ns_connection* to )
{
...
static int
wait_until_ready( sock_t sock, int for_read )
{
  fd_set set;
  FD_ZERO( &set );
  FD_SET( sock, &set );
...
static void
spawn_stdio_thread( sock_t sock, HANDLE hPipe, void* (*func)( void* ) )
{
...
static void
abs_path( const char* utf8_path, char* abs_path, size_t len )
{
...
static void
str_to_lowercase( char* d, const char* s )
{
  for( ; *s; ++s, ++d )
    *d = tolower( *s );
  *d = '\0';
...
static int
find_interpreter( const char* interps, const char* ext, char* res_buf, size_t buf_sz )
{
  // interps is ".ext {.ext }>cmd {.ext {.ext }>cmd}"
  // ext is ".ext"
  char* i, * e, * b, * p, * c, * z;
  size_t n;
  if( !interps || !ext || strlen(interps)<5 || strlen(ext)<2 ||
      interps[0]!='.' || ext[0]!='.' ) return 0;
  i = (char*)malloc( strlen(interps)+1 );
  if( !i ) return 0;
  str_to_lowercase( i, interps );
  e = (char*)malloc( strlen(ext)+2 );
  if( !e ) { free( i ); return 0; }
  str_to_lowercase( e, ext );
  strcat( e, " " );
  b = i;
  do { p = strstr( b, e ); b = p+2; } while( p && p>i && p[-1]!=' ' );
  if( !p ) { free( i ); free( e ); return 0; }
  c = strstr( p+strlen(e)-1, " >" );
  if( !c ) { free( i ); free( e ); return 0; }
  z = strstr( c+2, " ." );
  if( !z ) z = i + strlen(i);
  n = z - c - 2;
  if( n+1 > buf_sz ) { free( i ); free( e ); return 0; }
  memcpy( res_buf, interps+(c-i)+2, n );  // copy from original interps, not i
  res_buf[n] = '\0';
  free( i );
  free( e );
  return n;
...
static process_id_t
start_process( char* interp, const char* interps, const char* cmd,
    const char* env, const char* envp[], const char* dir, sock_t sock )
{
  ...
  char buf[MAX_PATH_SIZE], buf4[MAX_PATH_SIZE], buf5[MAX_PATH_SIZE],
      cmdline[MAX_PATH_SIZE], buf6[MAX_PATH_SIZE], * p;
  ...
  if( interp == NULL && (fp = fopen( cmd, "r" )) != NULL )
  {
    ...
    buf[sizeof(buf) - 1] = '\0';
    // G-: used to be only "#!"; I added also "NB." for J
    if( buf[0] == '#' && buf[1] == '!' )
      interp = buf + 2;
    else if( buf[0] == 'N' && buf[1] == 'B' && buf[2] == '.' )
      interp = buf + 3;
    fclose( fp );
    // G-: if not in first line, then try to use 'interps'
    if( interp == NULL && interps != NULL )
    {
      const char* ext = strrchr( cmd, '.' );
      if( find_interpreter( interps, ext, buf6, sizeof(buf6) ) > 0 )
        interp = buf6;
    }
    if( interp != NULL )
  ...
  to_wchar( buf5, full_dir, ARRAY_SIZE( full_dir ) );
...
static void
addenv2( struct cgi_env_block* blk, const char* name )
{
  const char* s;
...
static void
prepare_cgi_environment( struct connection* conn, const char* prog,
    struct cgi_env_block* blk )
{
  ...
  addenv( blk, "SERVER_SOFTWARE=%s-%s/%s-%s/%s", "Mongoose", MONGOOSE_VERSION,
      "NS", NS_SKELETON_VERSION, __DATE__ );
...
static void
open_cgi_endpoint( struct connection* conn, const char* prog )
{
  struct cgi_env_block blk;
  ...
  if( start_process( conn->server->config_options[CGI_INTERPRETER],
      conn->server->config_options[CGI_INTERPRETERS],
...
static void
on_cgi_data( struct ns_connection* nc )
{
...
static int
check_acl( const char* acl, uint32_t remote_ip )
{
  int allowed, flag;
  uint32_t net, mask;
  struct vec vec;
...
static int
is_valid_http_method( const char* s )
{
  return !strcmp( s, "GET" ) || !strcmp( s, "POST" ) || !strcmp( s, "HEAD" ) ||
      !strcmp( s, "CONNECT" ) || !strcmp( s, "PUT" ) || !strcmp( s, "PATCH" ) ||
...
static size_t
parse_http_message( char* buf, size_t len, struct mg_connection* ri )
{
  int is_request, n;
...
const char*
mg_get_header( const struct mg_connection* ri, const char* s )
{
  int i;
...
int
mg_match_prefix( const char* pattern, ssize_t pattern_len, const char* str )
{
  const char* or_str;
  int len, res, i, j;
  ...
  for( j = i = 0; i < pattern_len; ++i, ++j )
  {
    ...
    if( lowercase( &pattern[i] ) != lowercase( &str[j] ) )
      return -1;
  }
  return j;
}
...
void
mg_template( struct mg_connection* conn, const char* s,
...
must_hide_file( struct connection* conn, const char* path )
{
  ...
  if( mg_match_prefix( pw_pattern, strlen(pw_pattern), path ) > 0 )
    return 1;
#endif
  pattern = conn->server->config_options[HIDE_FILES_PATTERN];
  return pattern != NULL &&
...
static int
convert_uri_to_file_name( struct connection* conn, char* buf,
...
size_t
mg_write( struct mg_connection* c, const void* buf, size_t len )
{
...
void
mg_send_status( struct mg_connection* c, int status )
{
  struct connection* conn = MG_CONN_2_CONN( c );
  if( c->status_code == 0 )
  {
...
void
mg_send_header( struct mg_connection* c, const char* name, const char* v )
{
  struct connection* conn = MG_CONN_2_CONN( c );
  if( c->status_code == 0 )
  {
...
static void
terminate_headers( struct mg_connection* c )
{
  struct connection* conn = MG_CONN_2_CONN( c );
...
size_t
mg_send_data( struct mg_connection* c, const void* data, int data_len )
{
  struct connection* conn = MG_CONN_2_CONN( c );
  terminate_headers( c );
...
size_t
mg_printf_data( struct mg_connection* c, const char* fmt, ... )
{
  struct connection* conn = MG_CONN_2_CONN( c );
  va_list ap;
  int len;
...
static size_t
deliver_websocket_frame( struct connection* conn )
{
...
size_t
mg_websocket_write( struct mg_connection* conn, int opcode,
...
size_t
mg_websocket_printf( struct mg_connection* conn, int opcode,
    const char* fmt, ... )
{
...
static void
send_websocket_handshake_if_requested( struct mg_connection* conn )
{
...
static void
ping_idle_websocket_connection( struct connection* conn, time_t t )
{
...
static void
write_terminating_chunk( struct connection* conn )
{
  mg_write( &conn->mg_conn, "0\r\n\r\n", 5 );
}

static int
call_request_handler( struct connection* conn )
{
  int result;
...
const char*
mg_get_mime_type( const char* path, const char* default_mime_type )
{
  const char* ext;
  size_t i, path_len = strlen(path);
...
static void
get_mime_type( const struct mg_server* server, const char* path,
    struct vec* vec )
{
...
static void
construct_etag( char* buf, size_t buf_len, const file_stat_t* st )
{
...
static int
is_not_modified( const struct connection* conn,
...
static int
find_index_file( struct connection* conn, char* path,
...
static void
open_file_endpoint( struct connection* conn, const char* path,
    file_stat_t* st, const char* extra_headers )
{
      ...
      r1 >= 0 && r2 >= 0 )
  {
    ...
    conn->cl = n ==
        2 ? (r2 > conn->cl ? conn->cl : r2) - r1 + 1 : conn->cl - r1;
...
void
mg_send_file_data( struct mg_connection* c, int fd )
{
  struct connection* conn = MG_CONN_2_CONN( c );
  conn->endpoint_type = EP_FILE;
  conn->endpoint.fd = fd;
...
static void
call_request_handler_if_data_is_buffered( struct connection* conn )
{
#ifndef MONGOOSE_NO_WEBSOCKET
...
static int
scan_directory( struct connection* conn, const char* dir, struct dir_entry** arr )
{
...
static void
print_dir_entry( const struct dir_entry* de )
{
  ...
  else
  {
    ...
    if( fsize < 1000 )
      ...
      mg_snprintf( size, sizeof(size), "%d,%03d", (int)(fsize/1000),
          (int)(fsize%1000) );
    else if( fsize < G1 )
      mg_snprintf( size, sizeof(size), "%d,%03d,%03d",
          (int)(fsize/M1), (int)(fsize/1000%1000), (int)(fsize%1000) );
    else
      mg_snprintf( size, sizeof(size), "%d,%03d,%03d,%03d",
          (int)(fsize/G1), (int)(fsize/M1%1000), (int)(fsize/1000%1000),
          (int)(fsize%1000) );
  }
...
compare_dir_entries( const void* p1, const void* p2 )
{
  ...
  const struct dir_entry* b = (const struct dir_entry*)p2;
...
static void
send_directory_listing( struct connection* conn, const char* dir )
{
  ...
  mg_printf_data( &conn->mg_conn,
      ...
      "<tr><th align=left><a href=\"?n%c\">Name</a></th>"
      "<th><a href=\"?d%c\">Modified</a></th>"
      "<th align=right><a href=\"?s%c\">Size</a></th></tr>"
...
#ifndef MONGOOSE_NO_AUTH
void
mg_send_digest_auth_request( struct mg_connection* c )
{
  struct connection* conn = MG_CONN_2_CONN( c );
  c->status_code = 401;
  mg_printf( c,
...
static FILE*
open_auth_file( struct connection* conn, const char* path, int is_directory )
{
    ...
    const char* qop, const char* response )
{
  ...
  mg_md5( expected_response, ha1, ":", nonce, ":", nc, ":", cnonce, ":",
      qop, ":", ha2, NULL );
...
int
mg_authorize_digest( struct mg_connection* c, FILE* fp )
{
  struct connection* conn = MG_CONN_2_CONN( c );
  const char* hdr;
  ...
  while( fgets( line, sizeof(line), fp ) != NULL )
    if( sscanf( line, "%[^:]:%[^:]:%s", f_user, f_domain, ha1 ) == 3 &&
      ...
      return check_password( c->request_method, ha1, uri, nonce, nc, cnonce,
          qop, resp );
  return MG_FALSE;
}
...
static int
is_authorized( struct connection* conn, const char* path, int is_directory )
{
  FILE* fp;
...
static int
parse_header( const char* str, size_t str_len, const char* var_name,
...
int
mg_parse_header( const char* s, const char* var_name, char* buf, size_t buf_size )
{
...
static void
send_file_data( struct mg_connection* conn, FILE* fp )
{
  char buf[IOBUF_SIZE];
  size_t n;
    ...
    int include_level )
{
  ...
  if( include_level > 10 ) // MAX. SSI INCLUDE LEVEL
  {
...
static void
handle_ssi_request( struct connection* conn, const char* path )
{
  FILE* fp;
  struct vec mime_vec;
...
static void
proxy_request( struct ns_connection* pc, struct mg_connection* c )
{
...
#ifdef NS_ENABLE_SSL
int
mg_terminate_ssl( struct mg_connection* c, const char* cert )
{
...
int
mg_forward( struct mg_connection* c, const char* addr )
{
...
static void
proxify_connection( struct connection* conn )
{
...
#ifndef MONGOOSE_NO_FILESYSTEM
void
mg_send_file_internal( struct mg_connection* c, const char* file_name,
...
static void
open_local_endpoint( struct connection* conn, int skip_user )
{
#ifndef MONGOOSE_NO_FILESYSTEM
  char path[MAX_PATH_SIZE];
  file_stat_t st;
      ...
      mg_strncasecmp( conn->mg_conn.uri, "http", 4 ) == 0 )
  {
    ...
    if( enp == NULL || _stricmp( enp, "yes" ) != 0 )
...
#else
  {
    // G-: New feature in 5.6.7 -- shutdown
    const char* shutdown_cmd = conn->server->config_options[SHUTDOWN_COMMAND];
    if( shutdown_cmd && strcmp( conn->mg_conn.uri, shutdown_cmd ) == 0 )
    {
      mg_shutdown = 1; // this is supposed to be checked outside
      send_http_error( conn, 410, NULL );
      return;
    }
  }
...
static void
send_continue_if_expected( struct connection* conn )
{
...
static void
try_parse( struct connection* conn )
{
...
static void
do_proxy( struct connection* conn )
{
...
static void
on_recv_data( struct connection* conn )
{
  struct iobuf* io = &conn->ns_conn->recv_iobuf;
  int n;
...
static void
call_http_client_handler( struct connection* conn )
{
  // conn->mg_conn.status_code = code;
...
static void
process_response( struct connection* conn )
{
  struct iobuf* io = &conn->ns_conn->recv_iobuf;
...
mg_connect( struct mg_server* server, const char* addr )
{
...
log_access( const struct connection* conn, const char* path )
{
  ...
  if( path == NULL || (fp = fopen( path, "a+" )) == NULL ) return;
  now = time( NULL );
  // G-: I changed date/time format and removed [...] around it
  // strftime(date, sizeof(date), "%d/%b/%Y:%H:%M:%S %z", localtime(&now));
  strftime( date, sizeof(date), "%m/%d %H:%M:%S", localtime( &now ) );
...
static void
close_local_endpoint( struct connection* conn )
{
  struct mg_connection* c = &conn->mg_conn;
  // Must be done before free()
  int keep_alive = should_keep_alive( &conn->mg_conn ) &&
      (conn->endpoint_type == EP_FILE || conn->endpoint_type == EP_USER);
  DBG(( "%p %d %d %d", conn, conn->endpoint_type, keep_alive, conn->ns_conn->flags ));
...
static void
transfer_file_data( struct connection* conn )
{
  char buf[IOBUF_SIZE];
  size_t n;
...
void
mg_destroy_server( struct mg_server** server )
{
...
struct mg_connection*
mg_next( struct mg_server* s, struct mg_connection* c )
{
...
static int
get_var( const char* data, size_t data_len, const char* name,
...
int
mg_parse_multipart( const char* buf, int buf_len,
...
void
mg_copy_listeners( struct mg_server* s, struct mg_server* to )
{
  struct ns_connection* c;
  for( c = ns_next( &s->ns_mgr, NULL ); c != NULL;
      c = ns_next( &s->ns_mgr, c ) )
  {
...
static void
set_default_option_values( char** opts )
{
...
const char*
mg_set_option( struct mg_server* server, const char* name, const char* value )
{
...
static void
set_ips( struct ns_connection* nc, int is_rem )
{
  struct connection* conn = (struct connection*)nc->user_data;
  struct mg_connection* c = &conn->mg_conn;
  char buf[100];
...
static void
on_accept( struct ns_connection* nc, union socket_address* sa )
{
...
static void
process_udp( struct ns_connection* nc )
{
...
static void
mg_ev_handler( struct ns_connection* nc, int ev, void* p )
{
  struct connection* conn = (struct connection*)nc->user_data;
...
static void
iter2( struct ns_connection* nc, int ev, void* param )
{
...
void
mg_wakeup_server_ex( struct mg_server* server, mg_handler_t cb,
    const char* fmt, ... )
{
  va_list ap;
  char buf[8 * 1024];
...
void
mg_wakeup_server( struct mg_server* server )
{
  ns_broadcast( &server->ns_mgr, NULL, (void*)"", 0 );
}

const char*
mg_get_option( const struct mg_server* server, const char* name )
{
...
struct mg_server*
mg_create_server( void* server_data, mg_handler_t handler )
{
...
const char mg_features[] =
#ifdef MONGOOSE_NO_AUTH
  "-AU "
#else
  "+AU "
#endif
#ifdef MONGOOSE_NO_CGI
  "-CGI "
#else
  "+CGI "
#endif
  ...
  "-I6 "
#endif
#ifdef MONGOOSE_NO_LOGGING
  "-LOG "
#else
  "+LOG "
#endif
#ifdef MONGOOSE_NO_MMAP
  "-MM "
#else
  "+MM "
#endif
#ifdef MONGOOSE_NO_POPEN
  "-PO "
#else
  "+PO "
#endif
#ifdef MONGOOSE_NO_SSI
  "-SSI "
#else
  "+SSI "
#endif
